...
const Market = ()=>{
  ...
  useEffect(() => {
    ...
    .get(`https://j6a102.p.ssafy.io/api/v1/show/search`)
    .then((res) => {
  ...
  const getUserNickname = async (wallet) => {
    try {
      const response = await axios.get(
        `https://j6a102.p.ssafy.io/api/v1/profile/nickname/${wallet}`
      );
  ...
  const callShowDetail = async (address, id, name, poster_uri) => {
    try {
      ...
      var dateStartString = dateStart.getFullYear() + '.' + dateStart.getMonth() + '.' + dateStart.getDate()
      const endAt = await showScheduleContract.methods.getEndedAt().call();
      var dateEnd = new Date(endAt*1000)
      var dateEndString = dateEnd.getFullYear() + '.' + dateEnd.getMonth() + '.' + dateEnd.getDate()
      var now = new Date();
      ...
      SetShowList(showList => [...showList, {stageSellerName, stageName, dateStartString, dateEndString, price, id, name, poster_uri}]);
      SetShowListSearch(showListSearch => [...showListSearch, {stageSellerName, stageName, dateStartString, dateEndString, price, id, name, poster_uri}]);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const onSubmitCategory = async (newValue) => {
    ...
    .get(`https://j6a102.p.ssafy.io/api/v1/show/search?category_name=${newValue}`)
    .then((res) => {
      SetShowList([])
      SetShowListSearch([])
      for (let show of res.data) {
      ...
      <Grid container spacing={2}>
        <Grid item container spacing={0} xs = {2} direction="column">
          <Grid item container direction="row">
            <Grid item xs = {2}></Grid>
            <Grid item xs = {8}>
              <TextField id="search" label="크리에이터 또는 제목" variant="standard" onChange={searchKeyword}/>
            </Grid>
            <Grid item xs = {2}>
