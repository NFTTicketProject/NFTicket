  ...
  return (
    <>
      <PerformContainer>
        ...
        {isSeller ? (
          <TicketSellerName style={{ color: "black", fontWeight: 500 }}>내 티켓</TicketSellerName>
        ) : (
          <TicketSellerName style={{ fontSize: "14px", color: "grey" }}>
            {props.ticketSellerName}
          </TicketSellerName>
        )}
        {/* <p style={{ fontSize: "14px", color: "gray" }}>판매자 : {props.ticketSellerName}</p> */}
        <TicketSellerName style={{ fontSize: "14px", color: "gray", marginBottom: "12px" }}>
          {props.stageSellerName}
        </TicketSellerName>
        {new Date().getTime() > props.endAt * 1000 ? (
          <div>판매 종료</div>
        ) : (
          <div>{props.isEnded ? <div>판매 완료</div> : <div>판매 중</div>}</div>
        )}
